...
def getAnalitics():
    ...
    ax.bar(['Имена','Организации', 'Локации', 'Деньги', 'Даты'], height=[(sum(df["Имена"] == 1)/len(df)) * 100, 
                                   (sum(df["Организации"] == 1)/len(df)) * 100, 
                                   (sum(df["Локации"] == 1)/len(df)) * 100,
                                   (sum(df["Деньги"] == 1)/len(df)) * 100,
                                   (sum(df["Даты"] == 1)/len(df)) * 100,])
    ax.set_ylabel("Процент от общего использования")
    ax.set_title('Использование выделений в тексте')
    fig

...
def getAnalitics():
    ...
    ax1.bar(['Имена','Организации', 'Локации', 'Деньги', 'Даты'], height=[(sum(df["Имена"] == 1)/len(df)) * 100, 
    ...
    ax2.set_title('Количество обрабонного текста по проценту сокращение')
    ax2.set_xlabel("Процент сокращения")
    ax2.set_ylabel("Количество обработок по этому проценту")
    
    ax3.hist(data=df, x ="Процент сокращения", bins= 10)
    ax3.set_title('Количество обрабонного текста по проценту сокращение')
